...
export async function POST(request: NextRequest) {
  try {
    ...
    // Step 1: Validate timers exist, are running, belong to the user, and are not fixed-rate tasks
    ...
    // Exclude fixed-rate task timers; only pause hourly (and other non-fixed) task timers
    ...
    if (validTimers.length === 0) {
      return NextResponse.json({
        success: true,
        pausedCount: 0,
        message: 'No timers to pause (fixed-price task timers are excluded)',
      });
    }
