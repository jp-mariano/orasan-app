...
export function SubscriptionAction(props: {
  ...
  const data = useMemo(() => {
    const availablePlans: PlanWithPricing[] = plans
      ...
      .filter(plan => {
        return plan.pricing?.some(
          pricing => pricing.annual_price || pricing.monthly_price
        );
      })
      ...
      .map(plan => {
          ...
          .map(pricing => {
            ...
            return {
              ...
              updatePeriod: updatePeriod
                ? locale.billingCycle(updatePeriod).toLocaleLowerCase()
                : null,
            };
          });
        ...
        const closestPricing = findClosestPricing(
          filteredPricing,
          subscription.quota
        );
    ...
    const currentPlanIndex =
      availablePlans.findIndex(plan =>
        isIdsEqual(plan.id!, subscription.planId)
      ) ?? 0;
    ...
    const hasUpgrade =
      higherPlans.length > 0 || (currentPlan.pricing?.length ?? 0) > 1;
                      ...
                      <DropdownMenuItem
                        ...
                        onClick={() =>
                          checkout.open({
                            license_id: subscription.licenseId,
                            authorization:
                              subscription.checkoutUpgradeAuthorization,
                            plan_id: plan.id!,
