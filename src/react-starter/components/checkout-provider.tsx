...
function useCreateCheckout(
  ...
  const [fsCheckout, setFSCheckout] = useState<Checkout>(
...
export function CheckoutProvider({
  ...
  const syncPurchase = useCallback(
    async (purchaseData: CheckoutPurchaseData) => {
      const defaultSync: PurchaseSyncSuccess = async (
        purchaseData: CheckoutPurchaseData
      ) => {
        const url = getSanitizedUrl(endpoint);
        if (
          !url ||
          (!purchaseData?.purchase?.license_id &&
            !purchaseData?.trial?.license_id)
        ) {
          return;
        }
      ...
      try {
        const result = await (nestedContext?.success ?? onSync ?? defaultSync)(
          purchaseData
        );
    ...
    [
      ...
      onBeforeSync,
      onAfterSync,
      nestedContext?.success,
    ]
  );

  const fsCheckout = useCreateCheckout(
    checkout.options,
    syncPurchase,
    checkout.baseUrl
  );
  ...
  return (
    <CheckoutContext.Provider value={checkoutContext}>
      {isSyncing ? (
        <Processing>{message ?? locale.checkout.processing()}</Processing>
      ) : null}
      {children}
