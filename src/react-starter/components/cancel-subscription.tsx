    ...
    <>
      ...
      <div className="leading-relaxed mt-6">
        {(subscription.isTrial
          ? locale.portal.cancelSubscription.message.trial.paragraphs(
              daysLeft,
              null
            )
          : locale.portal.cancelSubscription.message.regular.paragraphs(
              daysLeft
            )
        ).map((p, i) => (
          ...
          onClick={() => {
            execute({ couponId: idToString(coupon.coupon_id!) }).then(() => {
              if (typeof window !== 'undefined') {
                window.alert(
                  locale.portal.cancelSubscription.alert.couponApplied()
                );
              }
...
function AskReasonAndCancel(props: {
  ...
  const [reasons, setReasons] = React.useState<
    NonNullable<SubscriptionCancellationRequest['reason_ids']>
  >([]);
  const [feedback, setFeedback] = React.useState<string>('');
  const { execute, loading } = usePortalAction<SubscriptionCancellationRequest>(
    subscription.cancelRenewalUrl
  );

  const hasTrial = subscription.isTrial;
  const reasonOptions = React.useMemo(
    () => getCancellationReasons(hasTrial),
    [hasTrial]
  );
  ...
  return (
    <>
      ...
      <div className="leading-relaxed mt-6">
        {locale.portal.cancelSubscription.message.reason
          .paragraphs()
          .map((p, i) => (
            <p className="mb-2" key={i}>
              {p}
      ...
      <div className="flex flex-col mt-6 gap-6">
        <div>
          <Label className="mb-3">
            {locale.portal.cancelSubscription.message.reason.form.reason()}
          </Label>
